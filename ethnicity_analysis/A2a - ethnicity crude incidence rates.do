/*==============================================================================
DO FILE NAME:			A3 - ethnicity crude incidence rates.do
...
DESCRIPTION OF FILE:	crude incidence rates by treatment and ethnic group
*=============================================================================*/

* Log
cap log close
log using $Logdir\A3_ethnicity_crude_incidence.log, replace
...
putexcel set "$Outputdir/Ethnicity crude incidence rates.xlsx", sheet("`outcome'") modify
...
********************************************************************************
	foreach treatmentgroup in su sglt2i dpp4i {
		...
		foreach ethnicity in white southasian black overall {
			...
			* Create end of follow-up for each outcome
			...
			if "`ethnicity'" == "overall" {		
				...
				keep if ethnicity == ``ethnicity''
			}

		* Row heading
		local x = ``treatmentgroup'_row' + ``ethnicity'_row'
		putexcel A`x' = "`ethnicity' `treatmentgroup' users"
...
********************************************************************************
	...
	* Number of CVD events
	count if `outcome'==1
	local number_events = `r(N)'
	putexcel B14 = `number_events', nformat("#")
	...
	* Median follow up years
	...
	local p75 = r(p75)
	local median_iqr = string(`median', "%9.1f") + " (" + string(`p25', "%9.1f") + "-" + string(`p75', "%9.1f") + ")"
	putexcel C14 = "`median_iqr'"

	* Total follow-up years
	summarize followup_years, meanonly
	putexcel D14 = `r(sum)', nformat("#.##")

	* Crude incidence rate
	local totalfu1000 = `r(sum)'/1000
	local rate = `number_events'/`totalfu1000'
	putexcel E14 = `rate', nformat("#.##") 
